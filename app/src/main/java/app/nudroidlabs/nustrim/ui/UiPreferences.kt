...
class UiPreferences(context: Context) {
    ...
    var interfaceMode: InterfaceMode?
        get() = preferences.getString(KEY_INTERFACE_MODE, null)?.let { stored ->
            runCatching { InterfaceMode.valueOf(stored) }.getOrNull()
        }
        set(value) {
            if (value == null) preferences.edit().remove(KEY_INTERFACE_MODE).apply()
            else preferences.edit().putString(KEY_INTERFACE_MODE, value.name).apply()
        }
